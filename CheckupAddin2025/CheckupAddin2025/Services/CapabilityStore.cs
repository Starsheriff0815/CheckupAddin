...
namespace CheckupAddIn.Services
{
    ...
    ///   2. AppData\Checkup 2025\Capabilities\  — personal unlocked copies.
    ///
    ...
    public class CapabilityStore
    {
        ...
        private readonly List<CapabilitySet>        _sets                  = new List<CapabilitySet>();
        ...
        private readonly string _localDir;        // AppData\Checkup 2025\Capabilities\
        ...
        public static CapabilityStore Load(string addinDir, string localDir)
        {
            ...
            // 1. Distribution files (addinDir\Capabilities\) — UNC path → auto-locked
        ...
        private void LoadFile(string filePath, bool fromDistribution = false)
        {
            try
            {
                ...
                var set  = JsonConvert.DeserializeObject<CapabilitySet>(json);
                if (set == null) return;
                if (string.IsNullOrEmpty(set.Id))
                    set.Id = Guid.NewGuid().ToString("N").Substring(0, 8);
        ...
        private void MigrateFromLegacy(string legacyPath)
        {
            try
            {
                ...
                var data = JsonConvert.DeserializeObject<LegacyCapabilityStoreData>(json);
        ...
        public CapabilitySet RevertToDistribution(CapabilitySet set)
        {
            ...
            if (_distributionFilePaths.TryGetValue(set.Id, out string distPath))
            {
                ...
                return _sets.Find(s => s.Id == set.Id);
            }
            return null;
        }
    ...
    {
        public List<CapabilitySet> CapabilitySets { get; set; } = new List<CapabilitySet>();
    }
}
