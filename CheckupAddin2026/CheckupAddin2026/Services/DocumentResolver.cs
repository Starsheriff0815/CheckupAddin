...
namespace CheckupAddIn.Services
{
    ...
    public class DocumentResolver
    {
            ...
            if (_app.ActiveDocument is AssemblyDocument asm)
            {
                ...
                for (int i = 1; i <= count; i++)
                {
                    try
                    {
                        ...
                        rawCounts[path] = rawCounts.TryGetValue(path, out int c) ? c + 1 : 1;
                        ...
                        if (!subAsmGroups.TryGetValue(parentIam, out var partDict))
                        {
                            partDict = new Dictionary<string, int>(StringComparer.OrdinalIgnoreCase);
                            subAsmGroups[parentIam] = partDict;
                        }
                        partDict[partName] = partDict.TryGetValue(partName, out int pc) ? pc + 1 : 1;
                    }
                    catch { }
                }
        ...
        private static string GetImmediateParentSubAsmFilename(ComponentOccurrence occ)
        {
            try
            {
                var parent = occ.ParentOccurrence;
