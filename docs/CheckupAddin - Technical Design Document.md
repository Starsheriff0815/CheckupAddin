...
| `SheetMetalReader`      | ⚠ **Backward-compat only.** Late-binding reads from `FlangeFeature` COM objects via `Microsoft.VisualBasic.Interaction.CallByName`. Both `SPECIAL:MiterGap` and `SPECIAL:FlangeDistance` are removed from the Field Selector catalog (2026-05-25) — they cannot be added to new rows. Resolver code is retained so that old presets still containing these field keys continue to display values. Do not remove and do not expand. |
...
- **Field Selector:** the ComboBox column on the right side of every Row. Full behaviour:
  ...
  **1. Special Functions** (German label: "Sonderfunktionen") — always the first group:
  - Contains: all Logic Constructor groups (`SPECIAL:LOGIC:`) where at least one Card or Basic Logic is **active** (toggled on). Label: `S: <GroupLabel>` with `S:` in red.
  - **Auto-collapse rule:** when all Logic Constructor groups have every Card and Basic Logic deactivated, the group has no entries → group **auto-collapses** and the collapse chevron is **disabled**. When at least one LC group becomes active: group auto-expands and chevron re-enables.
  - ⚠ **Legacy note:** `SPECIAL:MiterGap` ("Gehrungslücke") and `SPECIAL:FlangeDistance` were previously hardcoded entries in this group. Both were removed from the Field Selector catalog (2026-05-25). Resolver code is retained for backward-compat only — see §3 SheetMetalReader.
...
| R2 | Both | Card Enabled checkbox — checkbox is visible in the card row UI; disabled cards must be skipped by the engine.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | **DONE — INVENTOR CONFIRMED (2026-05-29).** `CardEngine` checks `!card.Enabled` at every card-type iteration. `CardRowVm.Enabled` setter updates model + fires `_onChanged` + `OnPropertyChanged()`. XAML `CheckBox IsChecked="{Binding Enabled}"`. Both projects confirmed working at runtime. |
...
| **Flange Distance**     | ⚠ Legacy. Computed sheet metal value (`SPECIAL:FlangeDistance`); removed from Field Selector catalog (2026-05-25); resolver retained for old presets only.                                                                                                                                                |
...
| **Miter Gap**           | ⚠ Legacy. Editable sheet metal value (`SPECIAL:MiterGap`, German: "Gehrungslücke"); removed from Field Selector catalog (2026-05-25); resolver retained for old presets only.                                                                                                                              |
...
- **R2 DONE:** Card Enabled checkbox confirmed working at Inventor runtime (both projects). Disabled cards correctly skipped by CardEngine at all card-type check points.
- **Audit fixes (both projects):** `CatalogBuilderViewModel.ExportCapSet` / `ImportCapSet` — empty catches replaced with `DiagLogger.Log`. `CheckupViewModel` (2026) — inline newline strip replaced with `DiagLogger.S()`.
- **Version:** `CheckupAddin2026.csproj` bumped to `0.9.8` (was `0.9.7`) to match 2024.
- **GitHub:** Repo live at https://github.com/Starsheriff0815/CheckupAddin. Tag `v0.9.8` pushed; GitHub Actions release build triggered.
