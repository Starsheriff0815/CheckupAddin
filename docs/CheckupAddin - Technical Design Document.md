...
| `SheetMetalReader`      | ⚠ **Backward-compat only.** Late-binding reads from `FlangeFeature` COM objects via `Microsoft.VisualBasic.Interaction.CallByName`. Both `SPECIAL:MiterGap` and `SPECIAL:FlangeDistance` are removed from the Field Selector catalog — they cannot be added to new rows. Resolver code is retained so that old presets still containing these field keys continue to display values. Do not remove and do not expand. |
...
| `SPECIAL:`              | —                         | **⚠ All hardcoded SPECIAL: entries removed from Field Selector.** Resolver code still handles existing keys for backward-compat only. No new hardcoded Special Functions shall be added — users build all derived fields via Logics-Constructor cards. |
| `SPECIAL:Spezi1`        | —                       | **⚠ Legacy.** IZ Spezifik 1 (backed by UDEF:SPEZIFIK1). Backward-compat only — replaced by Logics-Constructor capability set. |
| `SPECIAL:Spezi2`        | —                       | **⚠ Legacy.** IZ Spezifik 2 (backed by UDEF:SPEZIFIK2). Same as Spezi1.                                                                                         |
| `SPECIAL:HalbzeugName`  | —                       | **⚠ Legacy.** Halbzeug name field (backed by UDEF:ROHTEILNAME). Replaced by Logics-Constructor.                                                                      |
| `SPECIAL:HalbzeugIdent` | —                       | **⚠ Legacy.** Halbzeug ident field (backed by UDEF:ROHTEILIDENT). Replaced by Logics-Constructor.                                                                    |
...
- `SPECIAL:Spezi1/2` rows are **legacy** — they exist only in saved presets that were created before the Logics-Constructor replaced them. The "IZ Spezis Baukasten" pseudo-entry that previously injected these rows into the Field Selector was removed. New rows cannot be created with `SPECIAL:Spezi1/2` via the Field Selector; only old presets still referencing the key trigger the legacy code path.
...
| Far right | **Logics-Constructor button** | Opens the Logics-Constructor (CatalogBuilder) window; vertically aligned with the Field Selectors below in both position and width — looks like a continuation of the same column                                                                                        |
...
- **Field Selector:** the ComboBox column on the right side of every Row. Full behaviour:
  ...
  **1. Special Functions** (German label: "Sonderfunktionen") — always the first group:
  ...
  - ⚠ **Legacy note:** `SPECIAL:MiterGap` ("Gehrungslücke") and `SPECIAL:FlangeDistance` were previously hardcoded entries in this group. Both were removed from the Field Selector catalog. Resolver code is retained for backward-compat only — see §3 SheetMetalReader.
...
- **⚠ Legacy — MiterGap inline edit:** batch write supported on legacy rows; same error-collection pattern as Apply.
...
> **⚠ Legacy — skip unless debugging old presets.** All Spezi/Halbzeug hardcoded code was removed from both projects. Backward-compat resolver paths are the only remaining code. New development uses Logics-Constructor groups (`SPECIAL:LOGIC:`) exclusively. No new hardcoded `SPECIAL:` functions shall be added — the correct path is always composable cards.
>
...
|-----------------|-------------|-----------|-------------|---------|
| Main window ℹ button | `"MainAddin"` | `Win_Title_CheckupInfo` | 520 × 480 | Quick Guide: object selection, inline edit, right-click copy, drag reorder, preset management, auto-refresh |
...
| **Special Fields**      | `SPECIAL:`-prefixed fields. **Permanent:** `MiterGap` (active) and `FlangeDistance` (backward-compat; hidden from catalog). **⚠ REMOVED:** `Spezi1`, `Spezi2`, `HalbzeugName`, `HalbzeugIdent` — all code paths, XAML panels, and language keys removed from both projects. Logics-Constructor (`IZ_Spezis_Baukasten.capability.json`) is the replacement. For historical reference: these were UDEF-backed hardcoded fields with a dedicated picker window, replaced because the Logics-Constructor covers the same use case with full user configurability. | shown with "S:" tag                                                                      |
...
| **Flange Distance**     | ⚠ Legacy. Computed sheet metal value (`SPECIAL:FlangeDistance`); removed from Field Selector catalog; resolver retained for old presets only.                                                                                                                                                |
| **Group**               | One logic unit inside a Capability Set; corresponds to one `SPECIAL:LOGIC:` row / one `S:` entry in the Field Selector                                                                                                                                                                                         |
| **Header bar**          | The topmost row of the main window (above all data rows); contains "File:" label, Document Name Field, Logics-Constructor button                                                                                                                                                                            |
...
| **Miter Gap**           | ⚠ Legacy. Editable sheet metal value (`SPECIAL:MiterGap`, German: "Gehrungslücke"); removed from Field Selector catalog; resolver retained for old presets only.                                                                                                                              |
...
| **Spezi1 / Spezi2**     | ⚠ Legacy. The two IZ Spezifik fields (`SPECIAL:Spezi1` / `SPECIAL:Spezi2`); backed by `SPEZIFIK1/2`. Backward-compat layer only — replaced by Logics-Constructor groups.                                                                                                                                      |
