...
namespace CheckupAddIn.Services
{
    ...
    ///   2. AppData\Checkup 2027\Catalogs\  — personal unlocked copies created by the user.
    ///
    ...
    public class CatalogStore
    {
        ...
        private readonly List<CatalogData>          _catalogs              = new();
        ...
        private readonly string _localDir;       // AppData\Checkup 2027\Catalogs\
        ...
        private void LoadFile(string filePath, bool fromDistribution = false)
        {
            try
            {
                ...
                var catalog = JsonSerializer.Deserialize<CatalogData>(json, _opts);
                if (catalog == null) return;
                if (string.IsNullOrEmpty(catalog.Id))
                    catalog.Id = Guid.NewGuid().ToString("N")[..8];

                MigrateCapabilitySet_Unused(catalog); // no-op placeholder kept for symmetry
        ...
        public CatalogData RevertToDistribution(CatalogData catalog)
        {
            ...
            if (_distributionFilePaths.TryGetValue(catalog.Id, out string distPath))
            {
                ...
                return _catalogs.FirstOrDefault(c => c.Id == catalog.Id);
            }
            return null;
        }
        ...
        public void Save(CatalogData catalog)
        {
            ...
            try
            {
                var json = JsonSerializer.Serialize(catalog, _opts);
        ...
        /// (Kept for API symmetry with CapabilityStore.)
        /// </summary>
        ...
        // No-op placeholder — catalogs don't have a v1→v2 group migration like CapabilityStore.
        private static void MigrateCapabilitySet_Unused(CatalogData _) { }
    }

    internal class LegacyCatalogStoreData
    {
        public List<CatalogData> Catalogs { get; set; } = new();
    }
}
