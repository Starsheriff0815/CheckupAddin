...
namespace CheckupAddIn.Services
{
    ...
    public class FieldWriter
    {
        ...
        private readonly HashSet<Document> _batchDirty = new HashSet<Document>();
        ...
        public string WriteFieldValue(Document doc, string fieldKey, string newValue)
        {
            ...
            try
            {
                ...
                if (fieldKey.StartsWith("UDEF:"))
                    err = WriteUserDefinedProperty(doc, fieldKey.Substring("UDEF:".Length), newValue);
                else if (fieldKey.StartsWith("IPROP|"))
                    err = WriteStandardProperty(doc, fieldKey, newValue);
                else if (fieldKey.StartsWith("PARAM:User:"))
                    err = WriteParameter(doc, fieldKey.Substring("PARAM:User:".Length), newValue);
                else if (fieldKey.StartsWith("PARAM:Model:"))
                    err = WriteParameter(doc, fieldKey.Substring("PARAM:Model:".Length), newValue);
                else if (fieldKey.StartsWith("DOC:"))
                    err = WriteDocumentValue(doc, fieldKey.Substring("DOC:".Length), newValue);
                else
        ...
        ///
        /// Formula write (leading '='): probes Property.Expression first, then Property.Value = "=…"
        /// as a fallback, logging which path takes (DiagLogger "fx"). Returns null on success or an
        /// error string.
        /// </summary>
        private static string SetPropertyValueOrFormula(Property prop, string value, bool isFormula, string setName, string propName)
        {
            ...
            try
            {
                prop.Value = value;
                string back = TryReadExpression(prop);
                DiagLogger.Log("fx", $"WRITE formula via Property.Value — '{DiagLogger.S(setName)}.{DiagLogger.S(propName)}' = '{DiagLogger.S(value)}' → Value '{DiagLogger.S(prop.Value != null ? prop.Value.ToString() : null)}', Expression '{DiagLogger.S(back)}'");
                return null;
            }
            catch (Exception ex)
            {
